...
module.exports = function map(node, createe = create, addee = add) {
  let out, path = []

  walk(node, null, (snode, context) => {
    let tvalue = createe(snode)
    ...
    // Otherwize it's a node deeper in the hierarchy.
      ...
      // If iteratee returned a non null value we pass it to the addee.
      if (null != tvalue) {
        addee(tvalue, ptvalue.value)
      }
